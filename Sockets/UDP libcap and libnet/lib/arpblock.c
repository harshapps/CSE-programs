...
int main() {
  ...
  char errbuf[LIBNET_ERRBUF_SIZE], target_ip_addr_str[16],src_ip_addr_str[16];
  ...
  l = libnet_init(LIBNET_LINK, "eth1", errbuf);
  if ( l == NULL ) {
  ...
  scanf("%15s",src_ip_addr_str);

  src_ip_addr = libnet_name2addr4(l, src_ip_addr_str,\
      LIBNET_DONT_RESOLVE);
